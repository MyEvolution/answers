...
class StrVec
{
	...
	/*	StrVec & operator = (StrVec sv)//这里是拷贝或者移动传值 
		...
		string operator[](size_t n)//这个符号十分简陋！只是暂时用的 
		{
			return *(element+n);
		}
	private:
		string *element;//指向第一个string
		string *first_free;//指向尾后
		string *cap;//指向分配的内存的尾后 
		static allocator<string> alloc;//用来分配内存的 
 ...
 allocator<string> StrVec::alloc;//需要在外重新定义 
 ...
 /*	for(auto i = first_free;i!=element;)
 	...
 	for_each(element,first_free,[](string &s){alloc.destroy(&s);});//这里用不用写成常量引用？是不用的 因为我们包含的是string而非const string 
...
{
	auto i = alloc_n_copy(il.begin(),il.end());//这个放最前面 抛出异常时候原来数据还在 
	free();
	element = i.first;
	cap = first_free = i.second;
 return *this;
